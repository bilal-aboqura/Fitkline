...
import type { MylerzShipment, OrderStatus, StoredOrder } from "@/lib/order-store";
...
export function getMylerzConfiguration() {
  const apiUrl = (process.env.MYLERZ_API_URL?.trim() || DEFAULT_API_URL).replace(/\/$/, "");
  ...
  return {
    ...
    password,
    warehouseName,
    defaultWeightKg: Number.isFinite(parsedWeight) && parsedWeight > 0 ? parsedWeight : 1,
...
function mylerzError(response: MylerzResponse<unknown>, status: number) {
  const metadata = typeof response.ErrorMetadata === "string" ? ` (${response.ErrorMetadata})` : "";
...
async function getToken() {
  ...
  try {
    ...
    throw new MylerzIntegrationError("تعذر الاتصال بـMylerz حاليًا. جرّب مرة أخرى بعد قليل.");
  }
  ...
  if (!response.ok || !result.access_token) {
    throw new MylerzIntegrationError("تعذر تسجيل الدخول إلى Mylerz. راجع بيانات الربط.", response.status === 401 ? 401 : 502);
  }
  const seconds = Math.max(60, Number(result.expires_in) || 300);
  const value = `${result.token_type || "Bearer"} ${result.access_token}`;
  tokenCache = { value, expiresAt: Date.now() + Math.max(30, seconds - 60) * 1000 };
  return value;
}
...
function matchName(names: string[], candidate?: string) {
  ...
  return normalized && names.some((name) => {
...
async function getDestination(order: StoredOrder) {
  ...
  if (!location) {
    throw new MylerzIntegrationError("تعذر العثور على منطقة الطلب في إعدادات الشحن.", 400);
  }
  ...
  const names = [location.city.nameAr, location.city.nameEn, location.governorate.nameAr, location.governorate.nameEn];
  const city = zonesCache.value.find((item) => matchName(names, item.ArName) || matchName(names, item.EnName));
  const zone = city?.Zones?.find((item) => matchName(names, item.ArName) || matchName(names, item.EnName));
  if (!city?.Code) {
    throw new MylerzIntegrationError(
      `تعذر مطابقة «${location.governorate.nameAr}» مع مدن Mylerz. راجع قائمة المناطق في حساب Mylerz.`,
      400,
    );
  }
  return { cityCode: city.Code, ...(zone?.Code ? { zoneCode: zone.Code } : {}) };
}

function egyptianPhone(value: string) {
  const key = phoneComparisonKey(value);
  return key.length === 10 ? `0${key}` : value.trim();
}

function packageWeight(order: StoredOrder, fallback: number) {
  const weight = order.items.reduce((total, item) => {
    const kilograms = Number.parseFloat(item.sizeId);
    return total + (Number.isFinite(kilograms) ? kilograms * item.quantity : 0);
  }, 0);
...
export function orderStatusForMylerzStatus(status: string, current: OrderStatus): OrderStatus {
  const normalized = status.toLowerCase();
  if (normalized.includes("deliver")) return "completed";
  if (normalized.includes("cancel")) return "cancelled";
  if (normalized.includes("pick") || normalized.includes("transit") || normalized.includes("out for")) return "shipped";
...
export async function createMylerzShipment(order: StoredOrder): Promise<MylerzShipment> {
  if (order.mylerz?.trackingNumber) throw new MylerzIntegrationError("الطلب مربوط بشحنة Mylerz بالفعل.", 409);
  if (order.bosta?.trackingNumber) throw new MylerzIntegrationError("الطلب مربوط ببوسطة بالفعل. استخدم شركة شحن واحدة لكل طلب.", 409);
  if (order.subtotal === null || order.total === null) {
    throw new MylerzIntegrationError("لا يمكن إنشاء الشحنة قبل تأكيد أسعار الطلب والشحن.", 400);
  }
  const configuration = requiredConfiguration();
  const destination = await getDestination(order);
  const now = new Date();
  const pickupDueDate = new Date(now.getTime() + 24 * 60 * 60 * 1000).toISOString();
  const weight = packageWeight(order, configuration.defaultWeightKg);
  const description = order.items.map((item) => `${item.name} ${item.sizeLabel} × ${item.quantity}`).join("، ").slice(0, 200);
  const result = await mylerzJson<MylerzAddOrder>("/Orders/AddOrders", {
    method: "POST",
    body: JSON.stringify([{
      ...(configuration.warehouseName ? { WarehouseName: configuration.warehouseName } : {}),
      PickupDueDate: pickupDueDate,
      Package_Serial: 1,
      Reference: order.reference,
      Description: description || "Fitkline order",
      Total_Weight: weight,
      Service_Type: "DTD",
      Service: "ND",
      Service_Category: "DELIVERY",
      Payment_Type: order.paymentMethod === "cod" ? "COD" : "PP",
      COD_Value: order.paymentMethod === "cod" ? order.total : 0,
      Customer_Name: order.customer.name,
      Mobile_No: egyptianPhone(order.customer.phone),
      ...(order.customer.alternatePhone ? { Mobile_No2: egyptianPhone(order.customer.alternatePhone) } : {}),
      ...
      Address_Category: "H",
      Currency: "EGP",
      Special_Notes: [order.reference, order.notes].filter(Boolean).join(" — ").slice(0, 200),
      Pieces: [{ PieceNo: 1, Weight: weight }],
    }]),
  });
  const packageResult = result.Packages?.[0];
  if (!packageResult?.BarCode || packageResult.ErrorMessage || result.ErrorMessage) {
    throw new MylerzIntegrationError(packageResult?.ErrorMessage || result.ErrorMessage || "لم تُنشأ شحنة Mylerz.", 400);
  }
  return {
    trackingNumber: String(packageResult.BarCode),
    status: packageResult.Status?.trim() || "Uploaded",
    statusUpdatedAt: new Date().toISOString(),
    ...(result.PickupOrderCode ? { pickupOrderCode: String(result.PickupOrderCode) } : {}),
  };
}

export async function syncMylerzShipment(shipment: MylerzShipment): Promise<MylerzShipment> {
  const [statusResult, tracking] = await Promise.all([
    mylerzJson<{ Status?: string; BarCode?: string | number }>(`/Packages/GetPackageStatus?AWB=${encodeURIComponent(shipment.trackingNumber)}`),
    mylerzJson<MylerzTrack[]>("/Packages/TrackPackages", {
      method: "POST",
      body: JSON.stringify([{ Barcode: shipment.trackingNumber, ReferenceNumber: "" }]),
  ...
  return {
    ...shipment,
    status: statusResult.Status?.trim() || latest?.StatusArName?.trim() || shipment.status,
    statusUpdatedAt: statusTime(latest?.ChangedDate),
    ...(logs.length ? {
      timeline: logs.map((log) => ({
        status: log.StatusArName?.trim() || log.StatusEnName?.trim() || "تحديث حالة",
        ...(log.ChangedDate ? { changedAt: statusTime(log.ChangedDate) } : {}),
      })),
    } : {}),
  };
}
...
export async function downloadMylerzAwb(trackingNumber: string) {
  ...
  const bytes = Array.isArray(value)
    ...
    : Uint8Array.from(Buffer.from(value.replace(/^data:application\/pdf;base64,/, ""), "base64"));
  if (bytes.length < 4 || new TextDecoder().decode(bytes.slice(0, 4)) !== "%PDF") {
    throw new MylerzIntegrationError("رد Mylerz لا يحتوي على بوليصة PDF صالحة.");
  }
  return bytes;
}
